...
class PruningStrategy(ABC):
    ...
    All pruning strategies must implement the compute_scores method.

    Token selection priority (highest to lowest):
    1. keep_tokens: keep an exact number of tokens
    2. score_threshold: keep tokens with scores above the threshold
    ...
    def compute_scores(
        ...
        **ctx,
        ...
        Args:
            attention: Attention weights from the current layer [batch, heads, seq, seq]
            image_token_range: Position range of image tokens (start, end)
            config: VTR config
            **ctx: Extra context (e.g., prior_attention for some strategies)

        Returns:
            scores: Score for each image token [num_image_tokens]
        """
        pass
    ...
    def select_tokens(
        ...
        1. keep_tokens: keep an exact number of tokens
        2. score_threshold: keep tokens with scores > threshold
        ...
        # Priority 2: score_threshold (threshold mode)
            ...
            # If no token exceeds the threshold, keep at least the top-1
    ...
    def _aggregate_attention(
        ...
        if config.query_aggregation == "last":
            ...
            # Average attention from question tokens (after image, before last token) to image
        ...
        # Aggregate across heads based on head_aggregation
