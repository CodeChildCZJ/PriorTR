# Prunable LlamaModel
# LlamaModel implementation supporting multi-layer visual token pruning
...
class PrunableLlamaModel(LlamaModel):
    ...
    - Single-layer pruning: obtain attention at the specified layer and prune
    - Multi-layer pruning: prune sequentially at multiple layers, each using configured keep_ratio/keep_tokens

    Pruning flow:
    1. Obtain attention weights at layer K-1
    2. Use the strategy to compute scores and select tokens to keep
    3. Prune hidden_states, position_ids, attention_mask, and KV cache
    4. Continue forward through subsequent layers

    During multi-layer pruning, image_token_range is updated after each pruning step
    so that the next layer can correctly locate the remaining image tokens.
    """
    ...
    def setup_vtr(self, config: VTRConfig) -> None:
        """
        Set up VTR configuration and perform RoPE module replacement.
    ...
    def _replace_rope_with_unbounded(self) -> None:
        """
        [Critical Fix] Replace RoPE in all layers with the Unbounded version.
        Allows using original (larger) position_ids even when the physical sequence is shorter.

        Why this is needed:
        - After pruning, physical length is shorter (e.g., 190), but position_ids retain original values (up to 621)
        - Standard RoPE slices cache by physical length, causing out-of-bounds access for large position_ids
        - Unbounded RoPE returns the full cache, preventing out-of-bounds errors
        """
        for i, layer in enumerate(self.layers):
            old_rope = layer.self_attn.rotary_emb
            
            # Skip if already Unbounded type
    ...
    def _get_prune_layer_set(self) -> Set[int]:
        ...
        Returns:
            Set of layer indices that trigger pruning (layer K-1)
        """
        if self._vtr_config is None:
            return set()
        # prune_layers are the pruning layers; we need to get attention at layer K-1
    ...
    def forward(
        ...
        # VTR extra parameters (can be passed as arguments or obtained from injected state)
        ...
        Supports pruning at multiple layers; image_token_range is updated after each pruning step.
        """
        ...
        # [State Injection Support] If not provided as arguments, try to get from injected state
        ...
        # [Key: follow new HF logic] If first run and cache is needed, provide a cache object
        ...
        # =================================================================
        # [Fix 1] Dynamic RoPE cache pre-expansion
        ...
        if position_ids is not None and position_ids.numel() > 0:
            ...
            # Get the first layer's RoPE cache upper limit
            rope_module = self.layers[0].self_attn.rotary_emb
            # Compatible with different transformers version attribute names
        ...
        should_prune = (
            ...
            past_key_values_length == 0  # Only prune during prefill phase
        )
        
        # Get the set of layers that trigger pruning (layer K-1)
        ...
        # When pruning is needed, we need attention from layer K-1
        ...
        for decoder_layer in self.layers:
            ...
            if need_attention_for_pruning:
                # This layer must fall back to Eager with a 4D Mask to prevent causal leakage
                # Temporarily compute the 4D Mask for this layer
                ...
                # Other layers: use SDPA acceleration (Mask is typically None)
            ...
            # Prune immediately after layer K-1
                ...
                if attention_weights is None:
                    if self._use_flash_attention_2:
                        raise ValueError(
                            ...
                            "outputting attention weights. Please use 'sdpa' or 'eager' "
    ...
    def _compute_and_apply_pruning(
        ...
        # If there are no image tokens, return directly
        if num_img_tokens <= 0:
        ...
        # If keeping all tokens, return directly
        if num_kept_img_tokens >= num_img_tokens:
            return hidden_states, position_ids, attention_mask, past_key_values, seq_length, image_token_range
        
        # Step 3: Build complete keep_indices
        ...
        # New range: start position unchanged, end = start + number of kept tokens
        ...
        if attention_mask is not None:
            attention_mask = attention_mask.index_select(2, keep_indices).index_select(3, keep_indices).contiguous()
