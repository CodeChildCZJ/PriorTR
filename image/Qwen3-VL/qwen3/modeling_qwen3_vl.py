    ...
    def forward(
        ...
        **kwargs,
        ...
        if output_attentions:
            # Eager per-chunk attention with weight extraction
            # query/key/value_states shape: [1, H, seq_len, head_dim]
            ...
            for q, k, v in zip(q_chunks, k_chunks, v_chunks):
                # q, k, v: [1, H, N_i, head_dim]
                scores = torch.matmul(q, k.transpose(-1, -2)) * self.scaling  # [1, H, N_i, N_i]
                weights = torch.nn.functional.softmax(scores, dim=-1, dtype=torch.float32).to(q.dtype)
                out = torch.matmul(weights, v)  # [1, H, N_i, head_dim]
                attn_outputs.append(out.transpose(1, 2).contiguous())  # [1, N_i, H, head_dim]
                attn_weights_list.append(weights.squeeze(0))  # [H, N_i, N_i]

            attn_output = torch.cat(attn_outputs, dim=1)  # [1, seq_len, H, head_dim]
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        Args:
            ...
            output_attentions (`bool`, *optional*, defaults to `False`):
                Whether to return attention weights from the last vision block.
                When True, returns a list of `[H, N_i, N_i]` tensors (one per image).
        ...
        for layer_num, blk in enumerate(self.blocks):
            is_last = layer_num == len(self.blocks) - 1
            if output_attentions and is_last:
                hidden_states, last_attn_weights = blk(
                    hidden_states,
                    cu_seqlens=cu_seqlens,
                    position_embeddings=position_embeddings,
