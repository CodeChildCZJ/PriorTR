...
def parse_args():
    ...
    parser.add_argument("--model", default="gpt-3.5-turbo", type=str, help="Model name for API.")
...
def main():
    ...
    while True:
        try:
            ...
            # Files that have not been processed yet (excluding skipped files)
            incomplete_files = [f for f in caption_files if f not in completed_files and f not in skipped_files]
            ...
            # Check and update retry counts, skip files that exceeded max retries
            files_to_process = []
            for f in incomplete_files:
                retry_counts[f] = retry_counts.get(f, 0) + 1
                if retry_counts[f] > max_retries:
                    print(f"Skipping file '{f[:-5]}' after {max_retries} failed retries")
                    skipped_files.add(f)
                else:
                    files_to_process.append(f)

            # Update incomplete_files to only include files we'll process
            incomplete_files = files_to_process

            if len(incomplete_files) == 0:
                print(f"All remaining files skipped due to max retries. Skipped: {len(skipped_files)}")
                break
    ...
    if skipped_files:
        print(f"\nTotal skipped files due to parse errors: {len(skipped_files)}")
