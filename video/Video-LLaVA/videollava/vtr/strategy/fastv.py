...
class FastVStrategy(PruningStrategy):
    ...
    Selects Top-K image tokens based on mean attention from the last token.
    ...
    def compute_scores(
        ...
        **ctx,
        ...
        Args:
            attention: [batch, heads, seq_len, seq_len] attention weights at current layer
            image_token_range: (img_start, img_end)
            config: contains keep_ratio/keep_tokens etc.

        Returns:
            scores: [num_img_tokens] importance score for each image token
        """
        ...
        # attention shape: [batch, heads, q_len, k_len]
