...
try:
    ...
    def _expand_mask_bloom(attention_mask: torch.Tensor, tgt_length: Optional[int] = None):
        bsz, src_length = attention_mask.size()
        tgt_length = tgt_length if tgt_length is not None else src_length
        expanded = attention_mask[:, None, None, :].expand(bsz, 1, tgt_length, src_length).to(torch.bool)
        # True 表示 masked
        return ~expanded
    ...
    def _make_causal_mask_bloom(input_shape: Tuple[int, int], device: torch.device, past_key_values_length: int = 0):
        ...
        if past_key_values_length > 0:
            prefix = torch.zeros((tgt_len, past_key_values_length), device=device, dtype=torch.bool)
...
try:
    ...
    def _expand_mask_opt(attention_mask: torch.Tensor, dtype: torch.dtype, tgt_len: Optional[int] = None):
        bsz, src_len = attention_mask.size()
        tgt_len = tgt_len if tgt_len is not None else src_len
        expanded = attention_mask[:, None, None, :].expand(bsz, 1, tgt_len, src_len).to(dtype)
